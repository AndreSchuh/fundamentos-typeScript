...
// --- ROTAS PARA LIVROS (DESAFIO FINAL) ---

const validarLivro = (req: Request, res: Response, next: NextFunction) => {
    const { titulo, autor, anoPublicacao } = req.body;
    
    if (!titulo || !autor || !anoPublicacao) {
        return res.status(400).json({ error: "Título, autor e ano de publicação são obrigatórios." });
    }

    if (typeof anoPublicacao !== 'number' || anoPublicacao > new Date().getFullYear()) {
        return res.status(400).json({ error: "Ano de publicação inválido ou futuro." });
    }

    next();
};

app.post("/livros", validarLivro, async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { titulo, autor, anoPublicacao } = req.body;
...
app.get("/livros", async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (autor) {
            filtro.autor = { contains: autor as string, mode: 'insensitive' }; 
        }
        ...
        const livros = await prisma.livro.findMany({ where: filtro });
        res.json(livros);
    } catch (error) {
        next(error);
    }
...
app.patch("/livros/:id/emprestar", async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id } = req.params;
        ...
        const livroEmprestado = await prisma.livro.update({
            where: { id },
            data: { disponivel: false },
        });

        res.json(livroEmprestado);
    } catch (error) {
        next(error);
    }
});
...
// --- MIDDLEWARE DE ERRO E INICIALIZAÇÃO DO SERVIDOR ---
